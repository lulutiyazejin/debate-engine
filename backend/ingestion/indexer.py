...
from ingestion.confirm import ConfirmMixin
from ingestion.duplicate import (content_hash as _content_hash,
                                 doc_id_from_hash as _doc_id_from_hash,
                                 semantic_duplicate)
...
# 0.1.5 B3：内容哈希/语义查重出走 ingestion/duplicate.py（别名保留旧私名，调用点零改）


class Indexer(ConfirmMixin):
    ...
    # ---------- Stage 7-10：确认入库 ----------
    # 0.1.5 B3：confirm（Stage 7-10）与 _update_index 出走 ingestion/confirm.py（ConfirmMixin）
    ...
    def _semantic_duplicate(self, doc_id: str, doc_summary: str) -> dict | None:
        # 0.1.5 B3：实现出走 ingestion/duplicate.py，此处保留薄委派（调用方零改动）
        return semantic_duplicate(self.db, doc_id, doc_summary)
