...
### 0.17 可视化编辑感 · lieflat 对齐与字体管理（0.1.7）

- 视觉语法对齐 lieflat-charts：五视图编辑旁注三件套（N= / 轴值域 / 来源）；Mono/彩色双模（默认彩色，开关在 seg-row 右侧，Mono=灰阶明度+--accent 单一落点，同时作用力导向）；散点 -5..+5 可数刻度；刻度/百分比数字走 --mono；轻量入场动效（雷达中心缩放/散点错峰，沿用 120ms 公约）。
- 雷达 Glance 旁注：悬停 chip 列 |均值| 前三轴文字总结（借 8values 结论先行）。
- 3D 立方：「中心立场」相对坐标模式（夹壁+悬停真值）；「距离虚线」开关**默认关**，开启后中心→小立方虚线标真值；默认放大 R≈0.09；复位钮连中心重置。
- 雷达维度显示层合并 22→17（技术/AI、全球化/外交、经济正义、权力/组织、身份/性别 五组取均值，数据层 22 轴零迁移）+「展开细轴」开关回 22。
- 脉络风格统一：中文泳道/空泳道折叠/即时 tip/点击开档案/宽度适配容器/泳道垂直撑满/全不详时提示行。
- 进度条通栏三件套：模型卡独立通栏行「% · 已下/总 · 速度」，Ollama 原始状态仅留悬停。
- 字体外挂 UI 化：设置·字体分区（OFL 推荐字体多源续传下载/本地导入/应用/删除/许可展示，热生效）；canvas 文字 --sans token 化+fonts.ready 重绘，外挂字体覆盖画布标签。
- 力导向三态引导（未点/离线/有边）+「孤立节点」过滤 chip（借 obsidian graph view）。
