...
底部状态栏（QuotaBar 组件）常馻显示各 AI 服务商配额用量，白/黄/红三色预警。
...
| 组件 | 选型 | 理由 |
|------|------|------|
| 前端框架 | Tauri 2.x + React 18 | 独立桌面应用，唯一 UI 端 |
| 后端框架 | Python FastAPI | AI/NLP 生态最成熟 |
| 向量数据库 | LanceDB（本地/S3 双模式） | 原生 S3 备份，int8 量化 |
...
### V1.0 · 核心引擎（CLI 验证阶段）
- [ ] Python 后端：文档解析 + 向量化 + RAG 检索
- [ ] 立场分类器（自动 + 人工确认）
- [ ] 第一批 Skill 文件（5个立场 + 5个入库类型）
- [ ] 反驳生成（三种输出模式 × 八种风格）
- [ ] 命令行 CLI 验证检索质量

### V1.1 · 桌面 UI（独立桌面应用）
- [ ] Tauri + React 桌面窗口（三栏布局）
- [ ] 文档导入 UI（拖拽 + 立场确认流程）
- [ ] 知识库管理界面（文档列表、预览、删除）
- [ ] 反驳输出界面（流式显示 + 引用来源侧边栏）
- [ ] 新手引导流程 + 演示知识库
- [ ] BGE-M3 本地模型管理（下载/GPU加速/自检）
- [ ] API 服务商配置（Groq/Gemini/Cerebras/自定义）
- [ ] 底部配额状态栏（QuotaBar）
...
搜索结果展示三级粒度：文档→章节→具体段路（含黄色关键词高亮），每条结果匹配“查看原文”和“用作反驳来源”两个操作按鈕。
...
| `social_democracy.skill.md` | 社会民主主义 | 市场原教反对派、极左 |
...
| `historical_document.skill.md` | 历史文献、奕记 |
| `default.skill.md` | 通用兑退，所有类型均可处理 |

### 19.2 演示知识库（开笱就有）

预载入预处理好的 10-15 篇经典文献摘要，涵盖 5 个预置立场，让用户开笱就能体验完整的反驳工作流，无需导入任何文档。
...
政治敏感内容（帝国主义批判、极左派文本等）上传至 Google 服务器时，应在 UI 中显示提示，或自动降级到其他商��或本地模型。
...
Trace-Span 层次展示：
```
Trace: ingest_doc("咀耶克_通往奴役之路.pdf")
...
| `retried_style` | 切换风格重试 | 轿面：风格选择不符 |
| `retried_stance` | 切换立场重试 | 轿面：立场选择不符 |
| `discarded` | 生成后关闭界面 | 轿面：结果没被使用 |
...
⚠ 过去 7 天 RAG 质量趋势
  context_relevance 均値: 0.81  (+0.03 vs 上周)
  chunk_utilization 均値: 0.58  (-0.04 vs 上周)
...
设置页日志管理分区提供：当前日志整体大小显示、按类型单独清空按鈕。
