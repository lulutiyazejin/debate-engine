...
2. **回应引擎**：以数据库为弹药，对输入言论生成带引用的回应——0.1.2 五意图，0.1.4 起反驳/批判/评价三意图并入「回答」风格表（14 条），tab 收为 回答/分析/综合报告 三入口。
...
### 0.1.4 能力扩展（详见 PLAN-0.1.4.md）
- **数据继承根治**：数据目录与程序目录解耦开关（%APPDATA data-root.txt 覆盖）+ 设置内一键迁移（wal_checkpoint→带进度复制→旧目录保留）；安装器预置 skill 全文件存在即跳过，升级不吞用户改动。
- **组件中心**：外部能力全部设置内下载（BGE-M3 嵌入/OCR 包/docling 包），断点续传+sha256+代理三态+镜像源；四态卡片（未装/下载中/已装/禁用，借 ComfyUI-Manager）；BGE 装完引导全库重嵌入（向量表 embedding_model 圈定）。
- **统一阅读器**：查看与入库分家——md 转换只服务 AI 与归档，人看走原生格式（pdf.js 原页/SheetJS 解析自绘表格/mammoth docx/marked md/OCR 文字层），统一纸感外壳；转换档标 conversion: full/lossy/summary-only。
- **档案库**：knowledge_base/archive/{立场}/{作者}/ 同名 md+原件；复制/迁移/不归档三选可记忆；版次→年份→序号消撞名。
- **素材组**：素材篮升级为名组（公共素材组永置顶、删组并公共、存储无上限、单次注入预算 20）。
